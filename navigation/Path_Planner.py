...
def generateWallWaypoints(lenFD, lenAD, overlapAD, overlapFD, minX, maxX, minY, maxY, minAlt, maxAlt, distanceFromTarget):
        ...
        for wayPoint in  rightSideWayPoints:
             wayPoint[x], wayPoint[y], wayPoint[z] = wayPoint[z], wayPoint[x], wayPoint[y]
        ...
        for wayPoint in  bottomSideWayPoints:
             wayPoint[x], wayPoint[y], wayPoint[z] = wayPoint[x], wayPoint[z], wayPoint[y]
        ...
        wallSearchWaypoints = rightSideWayPoints +  bottomSideWayPoints + leftSideWayPoints + topSideWayPoints
        
        return wallSearchWaypoints
...
def main():
	...
	#Generate wall search points
        wallWayPoints = generateWallWaypoints(lenFD, lenAD, overlapAD, overlapFD, minX, maxX, minY, maxY, minAlt, maxAlt, distanceFromTarget)
        completeWayPointList = groundWayPoints + wallWayPoints
		
		
        #Write ground waypoints to txt file
        thefile = open('completeWayPointList.txt', 'w')

        for wayPoint in completeWayPointList:
                thefile.write("%s\n" % wayPoint)
        thefile.close()
        ...
        whichPlot = input('Type "ground" to plot ground searching way points or "wall" to plot wall searching way points: ')
        while(whichPlot != 'wall' and whichPlot != 'ground'):
                print('invalid input.... input "wall" or "ground"')
                whichPlot = input('Type "ground" to plot ground searching way points or "wall" to plot wall searching way points: ')
        if(whichPlot=='wall'):
                for wayPoints in wallWayPoints:
                        plt.plot([wayPoints[x]], [wayPoints[y]], 'bs')

                plt.axis([minX, maxX, minY, maxY])
                plt.show()

        else:#whichPlot is ground.
                #Ploting generated cordinates
                for wayPoints in groundWayPoints:
                        plt.plot([wayPoints[x]], [wayPoints[y]], 'bs')
                        currentAxis = plt.gca()
                        #rectangle x,y point of origin is bottom left rectangle
                        xCordCentRect = wayPoints[x]-(0.5*lenAD)
                        yCordCentRect = wayPoints[y]-(0.5*lenFD)
                        currentAxis.add_patch(Rectangle((xCordCentRect, yCordCentRect), lenAD, lenFD, fill=None))
